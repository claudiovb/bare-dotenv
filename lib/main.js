...
function _instructions (result, dotenvKey) {
  ...
  // Get environment
  let environment
  if (uri.searchParams && typeof uri.searchParams.get === 'function') {
    environment = uri.searchParams.get('environment')
  } else {
    // Fallback for environments where searchParams doesn't exist (like Bare)
    // Parse manually from the URL string
    const urlString = dotenvKey
    const envMatch = urlString.match(/[?&]environment=([^&]+)/)
...
function _configVault (options) {
  const debug = parseBoolean(env.DOTENV_CONFIG_DEBUG || (options && options.debug))
  const quiet = parseBoolean(env.DOTENV_CONFIG_QUIET || (options && options.quiet))
